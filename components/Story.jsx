...
// This component renders each Story card. It gets called from all three index.jsx file in pages directory.
const Story = ({ story }) => {
  return (
    <div className="w-full h-28 bg-mantis rounded-lg hover:shadow-xl mt-5 cursor-pointer transition transform duration-500 ease-in-out hover:shadow-md hover:-translate-y-1 hover:scale-105">
      <a
        ...
        className="inline-block w-full h-full flex items-center"
      >
        <div className="w-fit h-fit ml-5 flex flex-col">
          <div className="flex flex-row group">
            ...
            <span className="text-lg ml-2 group-hover:text-white transition duration-500 ease-in">
              {story.score}
            </span>
          </div>
        ...
        <div className={story.score > 99 ? "ml-5" : "ml-8"}>
          <p className="text-black font-ptSans text-xl font-medium">
          ...
          <div className="mt-2">
            <p className="text-lg">
              by <span className="text-redCrayola">{story.by}</span> on{" "}
              <span className="text-redCrayola">
                {moment.unix(story.time).format("MMMM DD, YYYY")}
              </span>
            </p>
          </div>
        </div>
